...
namespace EntityFrameworkCore.Collections.Extensions
{
    public static class QueryExtensions
    {
        ...
        public static DbSet<T> Take<T>(this DbSet<T> source,int count) => new DbSet<T>(new DbSetLogger(source.Proxy), new TakeExpression(source.Expression,count));
        ...
        public static T First<T>(this DbSet<T> source) => source.Proxy.Reader<T>(new FirstExpression(source.Expression, null)).FirstOrDefault();

        public static T First<T>(this DbSet<T> source, Expression<Func<T, bool>> predicate) => source.Proxy.Reader<T>(new FirstExpression(source.Expression, predicate)).FirstOrDefault();
